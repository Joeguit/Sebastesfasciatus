...
TC_2 = tempcorr(temp.tL2a, T_ref, pars_T);
...
% ------------------------------------------------------
numFirstIndInMatrix = 1;
numLastIndInMatrix = 12; 
lbl = '2a'; % label for the data matrix
%
L_0 = Lw0.(['tL',lbl]) * del_M; % cm, initial structural length 
[EL, EW] = get_ELW(allg, allLm, allome, f, kT_M, L_0, numFirstIndInMatrix, numLastIndInMatrix, time);
prdData.(['tL',lbl])(:,:) = EL/ del_M; 
prdData.(['tW',lbl])(:,:) = EW; 

% ------------------------------------------------------
numFirstIndInMatrix = numLastIndInMatrix + 1;
numLastIndInMatrix = numLastIndInMatrix + 12;
lbl = '2b';
%
L_0 = Lw0.(['tL',lbl]) * del_M; % cm, initial structural length 
[EL, EW] = get_ELW(allg, allLm, allome, f, kT_M, L_0, numFirstIndInMatrix, numLastIndInMatrix, time);
...
function [EL, EW] = get_ELW(allg, allLm, allome, f, kT_M, L_0, numFirstIndInMatrix, numLastIndInMatrix, time)
gmatrix = allg(numFirstIndInMatrix:numLastIndInMatrix); 
Lmmatrix = allLm(numFirstIndInMatrix:numLastIndInMatrix);
omematrix = allome(numFirstIndInMatrix:numLastIndInMatrix);

rho_B = 1 ./ 3./ (1 + f./ gmatrix); % -, scaled von Bert growth rate for individual
rT_B = kT_M .* rho_B; % d, von Bert growth rate at T for individual

L_i = f * Lmmatrix;  
L = L_i' - (L_i' - L_0) .* exp( - rT_B' .* time); % cm, expected length at time
EL = L; % cm, structural length
EW = L.^3 .* (1 + omematrix' .* f); % g,  wet weight
